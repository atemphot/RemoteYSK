...
public class RecognizerTask implements Runnable {
	...
	public RecognizerTask() {
		pocketsphinx
				.setLogfile("/data/data/com.wangling.remotephone/remotephone/sr/pocketsphinx.log");
		Config c = new Config();
		/*
		 * In 2.2 and above we can use getExternalFilesDir() or whatever it's
		 * called
		 */
		c.setString("-hmm",  "/data/data/com.wangling.remotephone/remotephone/sr/tdt_sc_8k");
		c.setString("-dict", "/data/data/com.wangling.remotephone/remotephone/sr/test.dic");
		c.setString("-lm",   "/data/data/com.wangling.remotephone/remotephone/sr/test.lm");
	...
	public void run() {
		...
		while (!done) {
			...
			case SHUTDOWN:
				...
				start_time = 0;
				//this.ps.endUtt();  This can not be called twice!!!!!!!!!!!!!
				this.audio = null;
				this.audio_thread = null;
				state = State.IDLE;
				done = true;
			...
			if (state == State.LISTENING) {
				...
				try {
					...
					if (hyp != null) {
						...
						if (hypstr != null && false == hypstr.equals(partial_hyp)) {
							//Log.d(getClass().getName(),
							//		"Hypothesis: " + hyp.getHypstr()
							//				+ " Uttid: " + hyp.getUttid()
							//				+ " Best_score: "
							//				+ hyp.getBest_score());
							if (this.rl != null && hyp != null && hyp.getHypstr() != null && use_partials) {
								Bundle b = new Bundle();
								b.putString("hyp", hyp.getHypstr());
								this.rl.onPartialResults(b);
							}
							changed_time = System.currentTimeMillis();
						}
						else if (hypstr != null && true == hypstr.equals(partial_hyp))
						{
							if (changed_time > 0 && System.currentTimeMillis() - changed_time > 500)
							{
								Log.d(getClass().getName(),
										"Hypothesis: " + hyp.getHypstr()
												+ " Uttid: " + hyp.getUttid()
												+ " Best_score: "
												+ hyp.getBest_score());
								if (this.rl != null && hyp != null && hyp.getHypstr() != null) {
									Bundle b = new Bundle();
									b.putString("hyp", hyp.getHypstr());
									this.rl.onResults(b);
								}
					...
					if (start_time > 0 && System.currentTimeMillis() - start_time > 30000)
					{
						if (this.rl != null && hyp != null && hyp.getHypstr() != null) {
							Bundle b = new Bundle();
							b.putString("hyp", hyp.getHypstr());

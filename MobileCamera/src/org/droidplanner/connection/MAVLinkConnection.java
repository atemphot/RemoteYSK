...
public abstract class MAVLinkConnection extends Thread {
	...
	private BufferedOutputStream getTLogFileStream()
			throws FileNotFoundException {
		File myDir = new File("/sdcard/mavlog/");
		myDir.mkdirs();
		File file = new File(myDir, "last.tlog");
		if (file.exists())
			file.delete();
	...
	private void saveToLog(MAVLinkPacket receivedPacket) throws IOException {
		if (logEnabled) {
			if (receivedPacket.msgid == 233) {
				Log.d("RTK", "saveToLog: MAVLINK_MSG_ID_GPS_RTCM_DATA");////Debug
			}
			try {
				logBuffer.clear();
